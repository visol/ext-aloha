...
*/
define(
['aloha/plugin', 'aloha/floatingmenu', 'i18n!characterpicker/nls/i18n', 'i18n!aloha/nls/i18n', 'css!characterpicker/css/characterpicker.css'],
	...
	CharacterOverlay.prototype = {
		...
		_initHideOnDocumentClick: function() {
			...
			$('body').click(function(e) {
				/* XXX: find a better solution to the e.target.localName condition. */
				var overlayVisibleAndNotTarget = (self.$node.css('display') === 'table') && 
												 (e.target != self.$node[0]) && 
												 (e.target.localName !== 'button'); // don't consider clicks to the 'show' button.
		...
		_initCursorFocus: function( onSelectCallback ) {
			...
			var movements = {
				...
				38: function up() {
					...
					if($prevRow.length > 0) {
						$prev = $($prevRow.children()[$current.index()]).addClass('focused');
						if($prev.length > 0) {
							$current.removeClass('focused');
						}
					}
				...
				40: function down() {
					...
					if($nextRow.length > 0) {
						$next = $($nextRow.children()[$current.index()]).addClass('focused');
						if($next.length > 0) {
							$current.removeClass('focused');
						}
					}
		...
		_createCharacterButtons: function($node, characters, onSelectCallback) {
			...
			function addRow() {
				return $('<tr></tr>').appendTo(self.$tbody);
			}
			var characterList = $.grep(
				characters.split(' '),
