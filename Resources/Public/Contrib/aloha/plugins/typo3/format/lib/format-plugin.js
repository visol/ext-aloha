...
define(
['aloha/plugin', 'aloha/floatingmenu', 'i18n!format/nls/i18n', 'i18n!aloha/nls/i18n', 'css!format/css/format.css'],
		...
		initButtons: function () {
			...
			jQuery.each(this.config, function(j, button) {
				switch( button ) {
					...
					case 'h6':
					case 'pre':
						if ( Aloha.settings.plugins.format[button]) {
							var len = Aloha.settings.plugins.format[button].length;
							for (var i = 0; i < len; i++) {
								var className = Aloha.settings.plugins.format[button][i];

								var str = ""+
								"that.multiSplitItems.push({" +
									"'name' : button + '-' + i," +
									"'tooltip' : i18n.t('button.' + button + '.tooltip'),"+
									"'iconClass' : 'aloha-button ' + i18n.t('aloha-button-' + button) + ' ' + button + '-' + \"" + className + "\","+
									"'markup' : jQuery('<'+button+' class=\"" + className + "\"></'+button+'>')," +
									"'click' : function() {" +
										"if (Aloha.activeEditable) {" +
											"Aloha.activeEditable.obj[0].focus();"+
										"}"+
										"Aloha.Selection.changeMarkupOnSelection(jQuery('<' + button + ' class=\"" + className + "\"></' + button + '>'));" +
									"}"+
								"});";
								eval(str);
							}
